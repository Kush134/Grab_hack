...
class Orchestrator:
    def __init__(self):
        ...
        self.agent_keywords = self._get_agent_keywords()

    def _discover_agents(self):
        """Dynamically discovers and instantiates agents from the 'agents' directory."""
        agents = {}
                ...
                try:
                    ...
                    for attr in dir(agent_module):
                        ...
                        if isinstance(cls, type) and issubclass(cls, BaseAgent) and cls is not BaseAgent:
                            ...
                            print(f"Discovered and loaded agent: {instance.name}")
                except (ImportError, AttributeError) as e:
                    print(f"Could not load agent from {module_name}: {e}")
        return agents

    def _get_agent_keywords(self):
        """Gets keywords from each agent's capabilities."""
        keywords = {}
        for agent_name, agent_instance in self.agents.items():
            keywords[agent_name] = agent_instance.get_capabilities()
        return keywords

    def select_agent(self, user_question):
        """Selects the best agent based on keywords in the user's question."""
        user_question_lower = user_question.lower()
        scores = {name: 0 for name in self.agents}

        for agent_name, keywords in self.agent_keywords.items():
            for keyword in keywords:
                if re.search(r'\b' + re.escape(keyword) + r'\b', user_question_lower):
                    scores[agent_name] += 1
        
        # Select agent with the highest score
        if any(s > 0 for s in scores.values()):
            best_agent_name = max(scores, key=scores.get)
            return self.agents[best_agent_name]
        
        # Default to a general agent if no specific keywords are found
        return self.agents.get("Fintech Navigator")