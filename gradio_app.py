...
# gradio_app.py
import gradio as gr
from fin.orchestrator import Orchestrator
...
def get_agent_response(user_question):
    """
    This function takes the user's question, selects the best agent,
    and returns the agent's name and its GPT-4o generated response.
    """
    if not user_question.strip():
        return "No agent selected", "Please ask a meaningful question."

    # Use the orchestrator to select the appropriate agent
    selected_agent = orchestrator.select_agent(user_question)

    agent_name = selected_agent.name
    # Get the dynamic response from the selected agent
    response = selected_agent.get_response(user_question)
    return agent_name, response

# Define the Gradio interface
iface = gr.Interface(
    fn=get_agent_response,
    inputs=gr.Textbox(lines=7, label="Your Question", placeholder="Ask me anything about fintech, blockchain, or investment strategies..."),
    outputs=[
        gr.Textbox(label="Selected Agent"),
        gr.Textbox(lines=10, label="Agent Response")
    ],
    title="🤖 Fintech Multi-Agent System",
    description="Ask a question and the right agent will respond. This system uses Azure GPT-4o to generate expert answers.",
    allow_flagging=None
)